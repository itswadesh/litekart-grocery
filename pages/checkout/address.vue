      ...
      <div class="p-2 mb-24 w-72">
        ...
        >
          <template v-slot="{ result: { error, data }, query, isLoading }">
            ...
            <div v-else-if="data">
              <div
                class="flex flex-wrap items-start justify-between p-2 mb-2 bg-white border rounded shadow-lg"
                v-for="(a, ix) in data.myAddresses.data"
                :key="a.id"
              >
  ...
  methods: {
    selectFirstAddress(x) {
      this.selectedAddress =
        x.myAddresses &&
        x.myAddresses.data &&
        x.myAddresses.data[0] &&
        x.myAddresses.data[0].id
      return x
    },
    addressChanged(e) {
